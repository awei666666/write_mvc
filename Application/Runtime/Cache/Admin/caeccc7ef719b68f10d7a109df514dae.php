					...
					<div data-example-id="striped-table" class="bs-example">
						<table class="table table-bordered table-hover">
							...
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td><?php if(($vo['is_system']) == "1"): ?><font color="red">是</font><?php else: ?>否<?php endif; ?></td>
								<td align="left">
									<a href="<?php echo U('edit_attachclass',array('id'=>$vo['id']));?>" class="btn btn-success btn-xs glyphicon glyphicon-pencil">&nbsp;修改</a>　
									<?php if(($vo['is_system']) != "1"): ?><span id="del"><a href="javascript:void()" class="btn btn-danger btn-xs glyphicon glyphicon-trash">&nbsp;删除</a></span><?php endif; ?>
								</td>
	...
	<script type="text/javascript">
		...
		function del_button() {
			...
			layer.confirm('确认删除？', {
				...
				$.post('<?php echo U("index");?>', {'option':'delall','str' : str}, function (data) {
		...
		$(document).on('click', '#del', function () {
			...
			layer.confirm('确认删除？', {
				...
				$.post('<?php echo U("index");?>', {'option':'delone','str':idz}, function (data) {
					console.log(data);
